...
```bash
curl -sSL https://raw.githubusercontent.com/phnx-labs/linear-cli/v0.23.0/install.sh | sh
```

Manual pin (same tag; no checksum — prefer `install.sh` above):

```bash
curl -o ~/.local/bin/linear https://raw.githubusercontent.com/phnx-labs/linear-cli/v0.23.0/linear
